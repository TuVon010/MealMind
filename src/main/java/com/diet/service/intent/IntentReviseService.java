...
public class IntentReviseService {
    ...
    public IntentResult revise(SessionState state, IntentResult result, String userInput) {
        ...
        // 规则二：已有推荐结果时，明确的承接表达统一按调整处理，继续沿用历史槽位并排除已推荐餐食。
        if (hasLastRecommendations(state) && containsAdjustContinuationKeyword(userInput)) {
            return new IntentResult(Intent.MEAL_ADJUST, safeSlots(safeResult), safeResult.confidence());
        }
        ...
        // 规则四：含多餐规划关键词时强制 MEAL_PLAN，避免 LLM 误判成普通推荐
        ...
        // 规则五：推荐意图低置信度时先进入澄清链路；健康风险已在上方优先处理，不在这里降级
    ...
    /** 判断用户是否明确承接上一轮推荐，要求换一项或再给一批。 */
    private boolean containsAdjustContinuationKeyword(String userInput) {
        if (userInput == null || userInput.isBlank()) {
            return false;
        }
        return containsAny(userInput, "换一个", "换个", "换一批", "再换", "再推荐", "再来一个", "再来几个", "还有别的吗", "其他的呢");
    }
