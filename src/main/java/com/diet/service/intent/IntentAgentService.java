...
public class IntentAgentService {
     ...
     */
    public IntentResult recognize(String sessionId,
                                  Long userId,
                                  String userInput,
        ...
        try {
            ...
            Msg response = agentTraceService.callAgent(sessionId, "IntentAgent", modelName,
                    agent, buildUserPrompt(userId, sessionId, userInput, knownSlots, memoryContext, slotOptions));
    ...
    private String buildUserPrompt(Long userId,
                                   String sessionId,
                                   String userInput,
                                   SlotBundle knownSlots,
                                   MemoryContext memoryContext,
                                   Map<String, List<String>> slotOptions) {
        MemoryContext memory = memoryContext == null ? MemoryContext.empty() : memoryContext;
        return """
                userId: %s
                sessionId: %s
                sessionSummary: %s
                ...
                sessionSummary、recentHistory 和 knownSlots 只作为默认背景；当前这一句有明确新要求时，以当前要求为准。
                """.formatted(userId, sessionId, memory.promptSafeSessionSummary(),
                memory.promptSafeRecentHistory(), knownSlots, slotOptions, userInput);
    }
